...
PMCCABE = /opt/releng/tools/pa-risc_linux/pmccabe/$(PMCCABE_VER)/$(shell GPOS_BIT=$(ARCH_BIT) $(BLD_TOP)/make/platform_defines.sh)
XERCES =  /opt/releng/tools/xerces/xerces-c/$(XERCES_VER)/$(shell GPOS_BIT=$(ARCH_BIT) $(BLD_TOP)/make/platform_defines.sh)
XERCES_LIBDIR = $(XERCES)/lib

LIBGPOS = /opt/releng/tools/emc/libgpos/$(LIBGPOS_VER)/$(shell GPOS_BIT=$(ARCH_BIT) $(BLD_TOP)/make/platform_defines.sh)/libgpos
...
CFLAGS_WARN = -Wall -Werror -Wextra -pedantic-errors -Wno-variadic-macros
CFLAGS_OPTIONAL = -Wconversion
...
CFLAGS = $(CFLAGS_WARN) $(CFLAGS_OPTIONAL) $(CFLAGS_DL) $(CFLAGS_ARCH) $(CFLAGS_TYPE) $(CFLAGS_EXTRA) $(CFLAGS_INLINE)
...
ifeq (Darwin, $(UNAME))
	LDLIBFLAGS = -dynamiclib  -Wl,-headerpad_max_install_names,-undefined,dynamic_lookup -current_version $(LIB_VERSION).$(BLD_VERSION) -compatibility_version $(LIB_COMPAT_VERSION) $(LDLIBS) $(LDFLAGS)
	LDSFX = dylib
else
	LDLIBFLAGS = -shared
	LDSFX = so
