...
if [ "${COVERITY_BUILD}" == "True" ]; then
    ...
    if [ $result_of_make != 0 ]; then
        ...
        cov-analyze --dir ${BLD_TOP}/coverity \
            ...
            --enable PARSE_ERROR \
            --enable BAD_SHIFT \
            --checker-option FORWARD_NULL:dynamic_cast:false \
	...
	if [ $? = 0 ]; then
	    ...
	    if [ $? = 0 ]; then
		mkdir -p ${PULSE_BASE_DIR}/optimizer/server/${ARCH_OUTPUT}.debug
		cp /opt/releng/build/${PULSE_PROJECT}/`basename ${PULSE_BASE_DIR}`/optimizer/server/${ARCH_OUTPUT}.debug/test_log.txt \
								 ${PULSE_BASE_DIR}/optimizer/server/${ARCH_OUTPUT}.debug/
	    else
		mkdir -p ${PULSE_BASE_DIR}/optimizer/server/${ARCH_OUTPUT}.opt
		cp /opt/releng/build/${PULSE_PROJECT}/`basename ${PULSE_BASE_DIR}`/optimizer/server/${ARCH_OUTPUT}.opt/test_log.txt \
								 ${PULSE_BASE_DIR}/optimizer/server/${ARCH_OUTPUT}.opt/
	    fi
	fi
fi
