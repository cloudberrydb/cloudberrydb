...
Feature: gpcheckcat tests
    ...
    Scenario: gpcheckcat should generate repair scripts when only -g option is provided
        ...
        And the user runs sql "set allow_system_table_mods=true; update gp_distribution_policy  set distkey=NULL where localoid='foo'::regclass::oid;" in "constraint_g_db" on all the segments
        Then psql should return a return code of 0
