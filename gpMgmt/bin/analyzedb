...
#
# This software contains the intellectual property of VMware, Inc. or its affiliates.
# or is licensed to VMware, Inc. or its affiliates. from third parties. Use of this
# software and the intellectual property contained therein is expressly
# limited to the terms and conditions of the License Agreement under which
# it is provided by or on behalf of VMware, Inc. or its affiliates.
...
# pygresql 5 automatically converts psql timestamp to type datetime.datetime.
# We need to cast it as text for easy comparison to the state file on disk
GET_REQUESTED_LAST_OP_INFO_SQL = """
    SELECT PGN.nspname, PGC.relname, objid, staactionname, stasubtype, cast(statime AS TEXT) FROM pg_stat_last_operation, pg_class PGC, pg_namespace PGN
    WHERE objid = PGC.oid
    AND objid in (%s)
...
# Get leaf partitions of named root table.
GET_LEAF_PARTITIONS_SQL = """
select n.nspname, c.relname
from pg_partition_tree(
  (select c.oid from pg_class c, pg_namespace n where n.oid = c.relnamespace and nspname='%s' and relname='%s' and not relispartition)
), pg_class c, pg_namespace n
where n.oid = c.relnamespace and relid = c.oid
"""

# Get mid-level partitions of table with given OID
GET_MID_LEVEL_PARTITIONS_SQL = """
select n.nspname, c.relname
from pg_class c, pg_namespace n
where n.oid = c.relnamespace
and c.relkind = 'p' and c.relispartition
"""
...
SELECT child_nsp.nspname, child_c.relname, root_nsp.nspname, root_c.relname
FROM pg_class child_c, pg_namespace child_nsp,
     pg_class root_c, pg_namespace root_nsp
WHERE child_c.relnamespace = child_nsp.oid
AND   root_c.relnamespace = root_nsp.oid
AND child_c.oid IN (%s)
AND root_c.oid = pg_partition_root(child_c.oid)
and root_c.oid <> child_c.oid
"""

GET_ALL_ROOT_PARTITION_TABLES_SQL = """
SELECT distinct n.nspname || '.' || c.relname
FROM pg_class c, pg_namespace n
WHERE n.oid = c.relnamespace AND c.relkind = 'p' AND NOT c.relispartition;
"""
...
def get_partition_state_tuples(pg_port, dbname, catalog_schema, partition_info):
    ...
    the modification counters over all ao segment files, on all QE nodes.
...
class AnalyzeDb(Operation):
    def __init__(self, options, args):
        ...
        if options.coordinatorDataDirectory is None:
            options.coordinatorDataDirectory = gp.get_coordinatordatadir()
    ...
    def _get_pgport(self):
        ...
        if not env_pgport:
            return self._get_coordinator_port(self.coordinator_datadir)
        return env_pgport
    ...
    def _get_input_tables(self, col_dict):
        ...
        if self.single_table:
            ...
            for line in cfg_file:
                ...
                is_root_part = False
                for row in all_root_partitions:
                    if orig_table in row:
                        is_root_part = True
                self._parse_column(col_dict, orig_table, schema, table, included_cols, excluded_cols, is_root_part)
    ...
    def _write_back(self, curr_ao_state, curr_last_op, prev_ao_state, prev_last_op, heap_partitions,
        ...
        validate_dir("%s/%s/%s/%s" % (self.coordinator_datadir, self.analyze_dir, self.dbname, current_time))
        ...
        if len(ao_state_output) > 0:
            ao_state_filename = generate_statefile_name('ao', self.coordinator_datadir, self.analyze_dir, self.dbname,
                                                        current_time)
        ...
        if len(last_op_output) > 0:
            last_operation_filename = generate_statefile_name('lastop', self.coordinator_datadir, self.analyze_dir,
        ...
        if len(prev_col_dict) > 0:
            col_state_filename = generate_statefile_name('col', self.coordinator_datadir, self.analyze_dir, self.dbname,
                                                         current_time)
        ...
        report_filename = generate_statefile_name('report', self.coordinator_datadir, self.analyze_dir, self.dbname,
                                                  current_time)
    ...
    def ensure_semaphore_file_exists(self):
        db_directory = "%s/%s/%s" % (self.coordinator_datadir, self.analyze_dir, self.dbname)
...
def get_prev_ao_state(timestamp, coordinator_datadir, analyze_dir, dbname):
    logger.debug("getting previous ao state...")
    prev_state_filename = generate_statefile_name('ao', coordinator_datadir, analyze_dir, dbname, timestamp)
...
def get_prev_last_op(timestamp, coordinator_datadir, analyze_dir, dbname):
    logger.debug("getting previous last operation...")
    old_pgstatlastoperation_file = generate_statefile_name('lastop', coordinator_datadir, analyze_dir, dbname, timestamp)
...
def get_prev_col_state(timestamp, coordinator_datadir, analyze_dir, dbname):
    logger.debug("getting previous column states...")
    prev_col_state_file = generate_statefile_name('col', coordinator_datadir, analyze_dir, dbname, timestamp)
...
def construct_entries_from_dict_aostate(ao_state_dict):
    ret = []
    for key, item in ao_state_dict.items():
        schema = key[0]
        table = key[1]
...
def construct_entries_from_dict_lastop(last_op_dict):
    ret = []
    for value in last_op_dict.values():
...
def construct_entries_from_dict_colstate(prev_col_dict):
    ret = []
    for schema_table, col_set in prev_col_dict.items():
...
def generate_statefile_name(type_str, coordinator_data_dir, analyze_dir, dbname, timestamp):
    use_dir = "%s/%s/%s/%s" % (coordinator_data_dir, analyze_dir, dbname, timestamp)
    if type_str == 'lastop':
...
def get_analyze_dirs(coordinator_datadir, statefile_dir, dbname):
    analyze_path = os.path.join(coordinator_datadir, statefile_dir, dbname)
