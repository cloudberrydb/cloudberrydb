...
CREATE TABLE reindex_crtabforadd_part_heap_btree (id int, date date, amt decimal(10,2)) 
DISTRIBUTED BY (id)
...
3: select 1 AS oid_same_on_all_segs_partition_new_idx from gp_dist_random('pg_class') WHERE relname = 'reindex_crtabforadd_part_heap_btree_1_prt_new_p_id_idx' GROUP BY oid having count(*) = (SELECT count(*) FROM gp_segment_configuration WHERE role='p' AND content > -1);
