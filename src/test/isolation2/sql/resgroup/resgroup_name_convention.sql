...
CREATE RESOURCE GROUP rgNameTest01 WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP rgNameTest01 SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP rgNameTest01;
CREATE RESOURCE GROUP __rg_name_test_01__ WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP Z WITH (cpu_hard_quota_limit=10);
DROP   RESOURCE GROUP Z;

-- max length is 63 characters
CREATE RESOURCE GROUP max012345678901234567890123456789012345678901234567890123456789 WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP max012345678901234567890123456789012345678901234567890123456789further WITH (cpu_hard_quota_limit=10);
...
s p a c e s
t	a	b	s" WITH (cpu_hard_quota_limit=10);
...
t	a	b	s";
-- punctuations
CREATE RESOURCE GROUP "!#$%&`()*+,-./:;<=>?@[]^_{|}~" WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "!#$%&`()*+,-./:;<=>?@[]^_{|}~" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "!#$%&`()*+,-./:;<=>?@[]^_{|}~";
-- quotation marks
CREATE RESOURCE GROUP "'' are 2 single quotation marks" WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "'' are 2 single quotation marks" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "'' are 2 single quotation marks";
CREATE RESOURCE GROUP """ is 1 double quotation mark" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP "0 as prefix" WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "0 as prefix" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "0 as prefix";
CREATE RESOURCE GROUP " leading space" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP "\\ are two backslashes" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP "\o162\o147\o61" WITH (cpu_hard_quota_limit=10);
CREATE RESOURCE GROUP "\x72\x67\x31" WITH (cpu_hard_quota_limit=10);
CREATE RESOURCE GROUP "\u0072\u0067\u0031" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP U&"\0441\043B\043E\043D" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP U&"\0072\0067\0031" WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "rg1" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "rg1";

-- CJK characters are allowed with or without double quotation marks
CREATE RESOURCE GROUP 资源组 WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "资源组" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP 资源组;
CREATE RESOURCE GROUP リソース・グループ WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "リソース・グループ" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP リソース・グループ;
CREATE RESOURCE GROUP 자원그룹 WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP "RG_NAME_TEST" WITH (cpu_hard_quota_limit=10);
CREATE RESOURCE GROUP rg_Name_Test WITH (cpu_hard_quota_limit=10);
CREATE RESOURCE GROUP "rg_name_test" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP "with" WITH (cpu_hard_quota_limit=10);
...
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "WITH";
CREATE RESOURCE GROUP "group" WITH (cpu_hard_quota_limit=10);
ALTER  RESOURCE GROUP "group" SET concurrency 2;
SELECT * FROM rg_name_view;
DROP   RESOURCE GROUP "group";
CREATE RESOURCE GROUP "create" WITH (cpu_hard_quota_limit=10);
...
CREATE RESOURCE GROUP 'must_fail' WITH (cpu_hard_quota_limit=10);

-- does not support leading numbers
CREATE RESOURCE GROUP 0_must_fail WITH (cpu_hard_quota_limit=10);
...
-- min length is 1 character
CREATE RESOURCE GROUP "" WITH (cpu_hard_quota_limit=10);
