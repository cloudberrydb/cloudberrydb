...
class GenSplitAgg(object):
    ...
    #lineitem1 only 1 distributed col
    T1 = {}
    T1['tablename'] = "lineitem1"
    T1['Distcol1'] = "l_partkey"
    #T1['Distcol2'] = "l_orderkey"
    T1['NonDistcol1'] = "l_suppkey"
    T1['NonDistcol2'] = "l_linenumber"

    #part2 created by joining part and partsupp on partkeys - 1 distributed col
    T2 = {}
    T2['tablename'] = "part1"
    ...
    #lineitem2 - 2 distributed columns
    T3 = {}
    T3['tablename'] = "lineitem2"
    T3['Distcol1'] = "l_partkey"
    T3['Distcol2'] = "l_orderkey"
    T3['NonDistcol1'] = "l_suppkey"
    T3['NonDistcol2'] = "l_linenumber"
    ...
    [ 'SK2_1' , 'SELECT %AGG1%(DISTINCT %DISTRIBUTED_COL1%) AS %DQANUM%_dqacol1 FROM %TABLE1% %SK8_OUTTER_QUERY% GROUP BY %NONDISTRIBUTED_COL1%' ],
    ...
    [ 'SK3_1' , 'SELECT %AGG1%(DISTINCT %DISTRIBUTED_COL1%) AS %DQANUM%_dqacol1, SUM(%DISTRIBUTED_COL1%) AS %DQANUM%_dqacol2 FROM %TABLE1% %SK8_OUTTER_QUERY% GROUP BY %NONDISTRIBUTED_COL1%' ],
    [ 'SK3_2' , 'SELECT %AGG1%(DISTINCT %DISTRIBUTED_COL1%) AS %DQANUM%_dqacol1, SUM(%NONDISTRIBUTED_COL1%) AS %DQANUM%_dqacol2 FROM %TABLE1% %SK8_OUTTER_QUERY% GROUP BY %NONDISTRIBUTED_COL2%' ],
    ...
    [ 'SK4_1' , 'SELECT %AGG1%(DISTINCT %DISTRIBUTED_COL1%) AS %DQANUM%_dqacol1, SUM(DISTINCT %DISTRIBUTED_COL1%) AS %DQANUM%_dqacol2 FROM %TABLE1% %SK8_OUTTER_QUERY% GROUP BY %NONDISTRIBUTED_COL1%' ]]
    ...
    SK10 = [ 'SK10_1' , 'SELECT %AGG1%(DISTINCT %NONDISTRIBUTED_COL1%) FROM %TABLE1% GROUP BY %DISTRIBUTED_COL2%' ]
    ...
    def makesql(self):
        ...
        #for SK10
        tcname_prefix = self.SK10[0]
        tcbody = self.SK10[1]
        for a in agg:
            thistestname = self.T3['tablename']+ '_' +tcname_prefix+ "_" +a
            query = tcbody
            query = query.replace('%AGG1%', a)
            query = query.replace('%TABLE1%', self.T3['tablename'])
            query = query.replace('%DISTRIBUTED_COL1%', self.T3['Distcol1'])
            query = query.replace('%DISTRIBUTED_COL2%', self.T3['Distcol2'])
            query = query.replace('%NONDISTRIBUTED_COL1%',  self.T3['NonDistcol1'])
            query = query.replace('%NONDISTRIBUTED_COL2%',  self.T3['NonDistcol2'])
            querylist.append([ thistestname , query])

        querylist.append([ self.T1['tablename']+ '_SK0_1_1' , 'SELECT SUM(DISTINCT ' +self.T1['Distcol1']+ ') FROM ' +self.T1['tablename'] ] )
        querylist.append([ self.T1['tablename']+ '_SK0_1_2' , 'SELECT COUNT(DISTINCT ' +self.T1['Distcol1']+ ') FROM ' +self.T1['tablename'] ] )
        querylist.append([ self.T1['tablename']+ '_SK0_2_1' , 'SELECT SUM(DISTINCT ' +self.T1['NonDistcol1']+ ') FROM ' +self.T1['tablename'] ] )
        querylist.append([ self.T1['tablename']+ '_SK0_2_2' , 'SELECT COUNT(DISTINCT ' +self.T1['NonDistcol1']+ ') FROM ' +self.T1['tablename'] ] )
    
        #create test files(sql and empty ans files)
        for x in querylist:
            query = x[1]
            skeleton = x[0]
            if 'SK2_1' in skeleton or 'SK0_' in skeleton:
                self.writefile(x[0], x[1], 'enumerate')
            else: 
                self.writefile(x[0], x[1], 'normal')    
            print query 

        
    def writefile(self, file, query, execmode = 'normal'):
        ...
        sqlfile.write("-- @executemode " +execmode+ '\n')
        sqlfile.write("--end_ignore\n")
        sqlfile.write(query+ ';\n')
        sqlfile.close()
        ansfile.close()
