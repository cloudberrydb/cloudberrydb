...
class GroupingFunctionTestDatabase(TINCTestDatabase):
    """
    database for grouping function
    """

    def __init__(self, database_name):
        super(GroupingFunctionTestDatabase, self).__init__(database_name)

    def setUp(self):
        # Call out to TINCTestDatabase.setUp which will return True if the database already exists
        if super(GroupingFunctionTestDatabase, self).setUp():
            return True
