...
class GpdbVerify:
    ...
    def gpcheckmirrorseg(self, options=None, master=False):
        ...
        if options is None:
            if master:
                options = " -mastermirror=true  -exclude recovery.conf -ignore 'gp_cdatabase' -ignore '_data' -ignore 'wet_execute.tbl' -ignore 'pg_xlog' -ignore 'pg_changetracking' -ignore 'pg_verify' -ignore 'backup_label.old' -dirignore 'pg_xlog' -ignore 'pg_subtrans' -ignore '/errlog/' -dirignore 'gpperfmon/data' "
            else:
                options = " -ignore '_data' -ignore 'wet_execute.tbl' -ignore 'core' -ignore pg_changetracking -ignore 'pg_xlog' -ignore 'pg_verify' -ignore '/errlog/' -parallel=true"
