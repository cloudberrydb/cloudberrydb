...
CREATE TABLE uao_table2(
          col_text text,
          col_numeric numeric
          CONSTRAINT tbl_chk_con1 CHECK (col_numeric < 250)
          ) with(appendonly=true) DISTRIBUTED by(col_numeric);

insert into uao_table2 values ('0_zero',0);
insert into uao_table2 values ('1_one',1);
insert into uao_table2 values ('2_two',2);
...
CREATE TABLE uao_table3 with(appendonly=true) as select * from uao_table2;
CREATE TABLE uao_table4 with(appendonly=true) as select * from uao_table1;
CREATE TABLE uao_table5 with(appendonly=true) as select * from uao_table1;
...
CREATE TABLE uaoschema1.uao_table7 with(appendonly=true) as select * from uao_table2;
Update uaoschema1.uao_table7 set col_text= 'new_0_zero' where col_numeric=0;

CREATE TABLE uao_table8 with(appendonly=true) as select * from uao_table1;
Delete from uao_table8 where bigint_col=0;

CREATE TABLE uao_table9 with(appendonly=true) as select * from uao_table1;
Delete from uao_table9 where bigint_col=0;
...
Create table uao_compr02 with(appendonly=true, compresstype='zlib', compresslevel=1, blocksize=8192) as select * from uao_compr01;
Update uao_compr02 set col_text='3_three' where col_numeric=3;
