...
BEGIN;
DECLARE 
    all_emp_uaocs_upd_rollback BINARY CURSOR FOR SELECT ename, loc FROM sto_uao_emp_forcursor  JOIN sto_uao_dept_forcursor
                               on dept = deptno order by ename ;
    FETCH FIRST from all_emp_uaocs_upd_rollback;
    update sto_uao_dept_forcursor set loc = 'NEW YORK CITY' where loc = 'NEW YORK';
    MOVE 5 FROM all_emp_uaocs_upd_rollback;
    FETCH  1 from all_emp_uaocs_upd_rollback ;
    CLOSE all_emp_uaocs_upd_rollback;
ROLLBACK;
...
BEGIN;
DECLARE 
    all_emp_uaocs_del_commit BINARY CURSOR FOR SELECT ename FROM sto_uao_emp_forcursor order by ename desc ;
    FETCH FIRST from all_emp_uaocs_del_commit;
    delete FROM sto_uao_emp_forcursor where empno > 10;
    FETCH 2 from all_emp_uaocs_del_commit;
    MOVE 5 FROM all_emp_uaocs_del_commit;
    FETCH  1 from all_emp_uaocs_del_commit ;
    FETCH NEXT from all_emp_uaocs_del_commit;
    FETCH all from all_emp_uaocs_del_commit;
    CLOSE all_emp_uaocs_del_commit;
COMMIT;

BEGIN;
DECLARE 
    all_emp_uaocs_aft_del CURSOR FOR SELECT ename FROM sto_uao_emp_forcursor order by ename desc ;
    FETCH all from all_emp_uaocs_aft_del;
    CLOSE all_emp_uaocs_aft_del;

COMMIT;
BEGIN;
DECLARE 
    all_emp_uaocs_upd_commit BINARY CURSOR FOR SELECT ename, loc FROM sto_uao_emp_forcursor  JOIN sto_uao_dept_forcursor
                               on dept = deptno order by ename ;
    FETCH FIRST from all_emp_uaocs_upd_commit;
    FETCH 2 from all_emp_uaocs_upd_commit;
    update sto_uao_dept_forcursor set loc = 'NEW YORK CITY' where loc = 'NEW YORK';
    MOVE 5 FROM all_emp_uaocs_upd_commit;
    FETCH  1 from all_emp_uaocs_upd_commit ;
    FETCH NEXT from all_emp_uaocs_upd_commit;
    FETCH all from all_emp_uaocs_upd_commit;
    CLOSE all_emp_uaocs_upd_commit;
COMMIT;
BEGIN;
...
BEGIN;
DECLARE 
    all_emp_uaocs_aft_upd BINARY CURSOR FOR SELECT ename, loc FROM sto_uao_emp_forcursor  JOIN sto_uao_dept_forcursor
                               on dept = deptno order by ename ;
    FETCH FIRST from all_emp_uaocs_aft_upd;
    FETCH 2 from all_emp_uaocs_aft_upd;
    MOVE 5 FROM all_emp_uaocs_aft_upd;
    FETCH  1 from all_emp_uaocs_aft_upd ;
    FETCH NEXT from all_emp_uaocs_aft_upd;
    FETCH all from all_emp_uaocs_aft_upd;
    CLOSE all_emp_uaocs_aft_upd;
COMMIT;
