...
SELECT position('pg_aocsseg_' in relname) FROM pg_class where oid IN (SELECT segidxid FROM pg_appendonly WHERE relid IN (SELECT oid FROM pg_class WHERE relname='t'));
