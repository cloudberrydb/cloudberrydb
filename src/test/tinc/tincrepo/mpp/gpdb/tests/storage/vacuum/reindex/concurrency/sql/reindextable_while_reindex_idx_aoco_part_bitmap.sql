...
DELETE FROM reindex_crtab_part_aoco_bitmap  WHERE id < 128;
1: BEGIN;
2: BEGIN;
1: REINDEX TABLE  reindex_crtab_part_aoco_bitmap;
2&: reindex index idx_reindex_crtab_part_aoco_bitmap_1_prt_de_fault;
1: COMMIT;
2<:
2: COMMIT;
3: select count(*) from reindex_crtab_part_aoco_bitmap where id = 998;
3: set enable_seqscan=false;
3: set enable_indexscan=true;
3: select count(*) from reindex_crtab_part_aoco_bitmap where id = 999;
