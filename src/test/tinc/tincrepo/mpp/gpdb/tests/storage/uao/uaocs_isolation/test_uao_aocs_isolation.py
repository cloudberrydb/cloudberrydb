...
class UAOCSIsolationTestCase(SQLIsolationTestCase):
    """
    @tags ORCA
    """

    '''
    Test for UAO AOCS tables with different concurrent transactions.
    '''
    sql_dir = 'sql/'
    ans_dir = 'expected'
    out_dir = 'output/'
    ...
    @classmethod
    def setUpClass(cls):
        super(UAOCSIsolationTestCase, cls).setUpClass()
        create_uao_udf()

    def get_output_substitution(self):
        return [(r'ERROR:  could not find segment file to use for inserting into relation (.+) \((.+)\). \((.+)\)',
            r'ERROR:  could not find segment file to use for inserting into relation \1 (****). (****)')]
