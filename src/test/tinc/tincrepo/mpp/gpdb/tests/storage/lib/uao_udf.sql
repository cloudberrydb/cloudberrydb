...
CREATE FUNCTION gp_aoseg_name(text) RETURNS TABLE (segno int, eof bigint, tupcount bigint, varblockcount bigint, eofuncompressed bigint, modcount bigint, state smallint) AS '$GPHOME/lib/postgresql/gp_ao_co_diagnostics.so','gp_aoseg_name_wrapper' LANGUAGE C STRICT;

DROP FUNCTION IF EXISTS gp_aocsseg_name(text);
CREATE FUNCTION gp_aocsseg_name(text) RETURNS TABLE (gp_tid tid, segno int4, column_num smallint, pysical_segno int, tupcount bigint, eof bigint, eof_uncompressed bigint, modcount bigint, state smallint) AS '$GPHOME/lib/postgresql/gp_ao_co_diagnostics.so','gp_aocsseg_name_wrapper' LANGUAGE C STRICT;
