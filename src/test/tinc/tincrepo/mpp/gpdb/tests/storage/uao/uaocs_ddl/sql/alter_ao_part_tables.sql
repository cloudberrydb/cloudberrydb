...
Drop table if exists sto_alt_ao_part;
Create table sto_alt_ao_part(
...
Create table sto_altap2 (a int, b int , c varchar(20)) with(appendonly=true, orientation=column) partition by range(a)
...
Alter table sto_alt_ao_part add partition p1 start(9) end(13) with(appendonly=true, orientation=column);
Insert into sto_alt_ao_part values(1,10,3,4);
select * from sto_alt_ao_part order by b,c;

-- Alter table add co partition
Alter table sto_alt_ao_part add partition p2 start(14) end(17) with(appendonly=true, orientation=column);
...
Alter table  sto_alt_ao_part alter partition p1 split partition FOR (RANK(1)) at(2) into (partition splita,partition splitb);
select * from sto_alt_ao_part order by b,c;
