...
--start_ignore
Drop table if exists sto_co_en_qlz;
--end_ignore
Create table sto_co_en_qlz (id SERIAL,
     a1 int ENCODING (compresstype=quicklz,compresslevel=1,blocksize=65536),
a2 char(5) ENCODING (compresstype=quicklz,compresslevel=1,blocksize=32768),
a3 text ENCODING (compresstype=quicklz,compresslevel=1,blocksize=8192)) WITH (appendonly=true, orientation=column) distributed randomly;

SELECT 1 AS VisimapPresent  FROM pg_appendonly WHERE visimapidxid is not NULL AND visimapidxid is not NULL AND relid=(SELECT oid FROM pg_class WHERE relname='sto_co_en_qlz');

Create index sto_en_qlz_idx1 on sto_co_en_qlz(a1);
Create index sto_en_qlz_idx2 on sto_co_en_qlz using bitmap(a3);

Insert into sto_co_en_qlz(a1,a2,a3) values(generate_series(1,10),'val1','Johnson & Johnsons McNeil Consumer Healthcare announced the voluntary dosage reduction today. Labels will carry new dosing instructions this fall');
Insert into sto_co_en_qlz(a1,a2,a3) values(generate_series(11,20),'val2','We are pleased to share the September schedule for onsite legal consultations with Duane Morris, our legal counsel for all Pivotal immigration matters.  Representatives will be onsite for one-on-one consultations regarding your individual case');

\d+ sto_co_en_qlz

select count(*) from sto_co_en_qlz;

Update sto_co_en_qlz set a3='Parents and other family members are always welcome at Stratford. After the first two weeks ofschool, we encourage you to stop b#%J,mbj756HNM&%.jlyyttlnvisiting, please berespectful of the classroom environment and do not disturb the students or teachers. Prior toeach visit, we require all visits to sign in at the school offbduysfifdsna v worn while visiting.As a safety precaution, the Stratford playgrounds are closed for outside visitation during normal school hours. We thank you for your cooperation.' where id=3;

select count(*) AS only_visi_tups  from sto_co_en_qlz;
set gp_select_invisible = true;
select count(*) AS invisi_and_visi_tups  from sto_co_en_qlz ;

set gp_select_invisible = false;
 
Delete from sto_co_en_qlz where a1>8 and a1<13;
...
VACUUM sto_co_en_qlz;

select count(*) AS only_visi_tups_vacuum  from sto_co_en_qlz;
set gp_select_invisible = true;
select count(*) AS invisi_and_visi_tups_vacuum  from sto_co_en_qlz;
set gp_select_invisible = false;
