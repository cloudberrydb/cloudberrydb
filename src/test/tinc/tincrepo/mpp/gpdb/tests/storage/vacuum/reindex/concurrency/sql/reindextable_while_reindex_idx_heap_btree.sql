...
2&: REINDEX TABLE  reindex_crtab_heap_btree;
1: COMMIT;
2<:
2: COMMIT;
3: select count(*) from reindex_crtab_heap_btree where a = 1000;
...
3: SELECT 1 AS relfilenode_same_on_all_segs from gp_dist_random('pg_class')   WHERE relname = 'reindex_crtab_heap_btree' GROUP BY relfilenode having count(*) = (SELECT count(*) FROM gp_segment_configuration WHERE role='p' AND content > -1);
