...
DELETE FROM reindex_crtab_part_ao_gist  WHERE id < 128;
1: BEGIN;
2: BEGIN;
1: REINDEX TABLE  reindex_crtab_part_ao_gist;
2&: reindex index idx_reindex_crtab_part_ao_gist_1_prt_de_fault;
1: COMMIT;
2<:
2: COMMIT;
3: select count(*) from reindex_crtab_part_ao_gist where id = 998;
3: set enable_seqscan=false;
3: set enable_indexscan=true;
3: select count(*) from reindex_crtab_part_ao_gist where id = 999;
