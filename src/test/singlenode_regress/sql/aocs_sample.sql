CREATE TABLE test_aocs_tablesample_append (dist int, id int, name text) WITH (appendonly=true, orientation=column);
...
CREATE TABLE ttr1_aocs_append (a int, b int)  with(appendonly=true, orientation=column);
CREATE TABLE ttr2_aocs_append (a int, b int)  with(appendonly=true, orientation=column);
