...
-- Lots of connections
CREATE FUNCTION icudp_history_test() RETURNS void LANGUAGE plpgsql READS SQL DATA AS $$
DECLARE
    local_jkey INT;
    local_val  INT;
BEGIN
...
CREATE FUNCTION system_call_fault_injection_test() RETURNS void LANGUAGE plpgsql READS SQL DATA AS $$
DECLARE
    local_jkey INT;
    local_val  INT;
BEGIN
...
-- inject faults (FINC_OS_CREATE_THREAD) to pthread_create (this tests introduce a lot fatal errors)
SET gp_udpic_fault_inject_bitmap = 2097152;
SELECT system_call_fault_injection_test();

-- inject faluts FINC_OS_MEM_INTERFACE
...
CREATE FUNCTION query_cancel_fault_injection_test() RETURNS void LANGUAGE plpgsql READS SQL DATA AS $$
DECLARE
    local_val  BIGINT;
BEGIN
    FOR k IN 1 .. 90
