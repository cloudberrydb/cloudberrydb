...
select pg_relation_size('vfheapbm') from gp_dist_random('gp_id') where gp_segment_id = 2;
select pg_relation_size('pg_bitmapindex.pg_bm_' || 'ivfheapbm'::regclass::oid::text) from gp_dist_random('gp_id') where gp_segment_id = 2;
