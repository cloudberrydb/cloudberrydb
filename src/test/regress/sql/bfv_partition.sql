...
-- TEST
select count_operator('explain select cust_type, subscription_status,count(distinct subscription_id),sum(voice_call_min),sum(minute_per_call) from mpp7980 where month_id =\'2009-04-01\' group by rollup(1,2);','SIGSEGV');
...
select count_operator('explain select * from mpp24151_t, mpp24151_pt where tid = ptid and pt1 = \'hello0\';','Result');
...
select count_operator('explain select * from ds_4 where month_id = \'200800\';','Partition Selector');
...
select count_operator('explain select * from ds_4 a1,ds_4 a2 where a1.month_id = a2.month_id and a1.month_id > \'200800\';','Partition Selector');
