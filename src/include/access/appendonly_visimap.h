...
{
	/*
	 * Memory context to use for all visibility map related
	 * allocations.
	 */ 
	MemoryContext memoryContext;

	/*
	 * Information about the current visibility map entry.
	 * Each visibility map entry corresponds to a tuple in
	 * the visibility map table.
	 */ 
	AppendOnlyVisimapEntry visimapEntry;

	/*
	 * Support operations to search, load, and store
	 * visibility map entries.
	 */ 
...
typedef struct AppendOnlyVisimapDelete
{
	 ...
	 * currently stored in the spill file.
	 * This means that we store in-memory around 20 byte per visimap entry.
	 * The resulting overhead is in the area of 1MB per 1 billion rows.
	 */
	HTAB *dirtyEntryCache;

	/*
	 * A workfile storing the updated visimap entries.
	 * It is a consequtive list of dirty (compressed) visimap bitmaps that
	 * needs to be updated in the visimap later.
	 */
