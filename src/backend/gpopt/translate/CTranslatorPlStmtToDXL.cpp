	...
	)
{
			...
			plcHashExprType, plHashExprTypes)
	{
		...
		CDXLNode *pdxlnHashExpr = GPOS_NEW(m_pmp) CDXLNode(m_pmp, GPOS_NEW(m_pmp) CDXLScalarHashExpr(m_pmp, GPOS_NEW(m_pmp) CMDIdGPDB(oid)));
	...
	)
{
	...
	for (ULONG ul = 0; ul < ulNumCols; ul++)
	{
		...
		CMDIdGPDB *pmdidSortOp = GPOS_NEW(m_pmp) CMDIdGPDB(oidSortOpId);
