						   ...
						   PersistentStoreSharedData *storeSharedData)
{
	...
	/*
	 * Note: Originally the below IF STMT was guarded with a InRecovery flag
	 * check. However, this routine should not be called during recovery since
	 * the entries are not consistent...
	 */
	Assert(!InRecovery);
	...
	if (globalSequenceNum < storeSharedData->maxInUseSerialNum)
	{
		 ...
		 *
		 * Use the gp_persistent_repair_global_sequence GUC to get the system
		 * up.
		 */
...
HeapTuple
PersistentStore_GetScanTupleCopy(
								 PersistentStoreScan *storeScan)
{
...
inline static void
XLogRecPtr_Zero(XLogRecPtr *xlogLoc)
{
	MemSet(xlogLoc, 0, sizeof(XLogRecPtr));
}

void
PersistentStore_FlushXLog(void)
{
...
int64
PersistentStore_MyHighestSerialNum(
								   PersistentStoreData *storeData)
{
...
int64
PersistentStore_CurrentMaxSerialNum(
									PersistentStoreSharedData *storeSharedData)
{
 ...
 /* TID of the stored tuple. */
{
	...
	/*
	 * In order to keep the tuples the exact same size to enable direct reuse
	 * of free tuples, we do not use NULLs.
	 */
	...
	/*
	 * Return the TID of the INSERT tuple. Return the XLOG location of the
	 * INSERT tuple's XLOG record.
	 */
						  ...
						  HeapTuple *tupleCopy)
{
				   ...
				   &tuple, &buffer, false, NULL))
	{
		*tupleCopy = heaptuple_copy_to(&tuple, NULL, NULL);
		...
		/*
		 * In order to keep the tuples the exact same size to enable direct
		 * reuse of free tuples, we do not use NULLs.
		 */
		nulls = (bool *) palloc(storeData->numAttributes * sizeof(bool));
