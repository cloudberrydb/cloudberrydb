				 ...
				 uint32 *speculativeToken)
{
	...
	/*
	 * You shouldn't have been able to create a unique index on an
	 * AO table in the first place, but let's santiy check.
	 */
	if (RelationIsAppendOptimized(heapRel))
		elog(ERROR, "append-only tables do not support unique indexes");
