...
static void check_ivm_restriction(Node *node, bool defer);
...
static bool check_aggregate_supports_ivm(Oid aggfnoid, bool defer);
				  ...
				  QueryCompletion *qc)
{
	...
	if (is_matview && into->ivm)
	{
		...
		check_ivm_restriction((Node *) query, into->defer);
		/* close orca for rewrite agg functions */
		optimizer = false;
		/* For IMMV, we need to rewrite matview query */
		query = rewriteQueryForIMMV(query, into->colNames, into->defer);
	...
	if (is_matview)
	{
		...
		if (into->ivm)
		{
			...
			if (!into->skipData || into->defer)
			{
				Assert(query_immv != NULL);
				/* Create triggers on incremental maintainable materialized view */
				CreateIvmTriggersOnBaseTables(query_immv, matviewOid, into->defer, true);
				if (into->defer)
					ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
									errmsg("incrementally maintainable materialized view with deferred refresh is not supported yet")));
			}
			table_close(matviewRel, NoLock);
		}
	}
 ...
 */
Query *
rewriteQueryForIMMV(Query *query, List *colNames, bool defer)
{
	Query *rewritten;
 ...
 */
void
CreateIvmTriggersOnBaseTables(Query *qry, Oid matviewOid, bool defer, bool create)
{
	Relids	relids = NULL;
	bool	ex_lock = false;
	RangeTblEntry *rte;
 ...
 */
static void
check_ivm_restriction(Node *node, bool defer)
{
	check_ivm_restriction_context context = {false, defer};
		...
		case T_Query:
			{
				...
				if ((qry->groupClause == NIL || !qry->hasAggs) && context->defer)
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				...
				foreach(lc, qry->rtable)
				{
					...
					if (rte->relkind == RELKIND_RELATION && has_superclass(rte->relid) && !context->defer)
						ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		...
		case T_JoinExpr:
			{
				...
				if (joinexpr->jointype > JOIN_INNER || context->defer)
						ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		...
		case T_Aggref:
			{
				...
				if (!check_aggregate_supports_ivm(aggref->aggfnoid, context->defer))
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
 ...
 */
static bool
check_aggregate_supports_ivm(Oid aggfnoid, bool defer)
{
	switch (aggfnoid)
	{
		/* count */
