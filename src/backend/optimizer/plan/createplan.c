...
static RuntimeFilter *create_runtime_filter_plan(PlannerInfo *root,
												 RuntimeFilterPath *best_path);
...
create_plan_recurse(PlannerInfo *root, Path *best_path, int flags)
{
	...
	switch (best_path->pathtype)
	{
		...
		case T_RuntimeFilter:
			plan = (Plan *) create_runtime_filter_plan(root,
													   (RuntimeFilterPath *) best_path);
			break;
		case T_Limit:
...
create_modifytable_plan(PlannerInfo *root, ModifyTablePath *best_path)
{
	...
	/*
	 * GPDB_14_MERGE_FIXME: subpaths is removed from ModifyTablePath
	 * need to handle multiple subpaths for sri?
	 */
...
create_runtime_filter_plan(PlannerInfo *root, RuntimeFilterPath *best_path)
{
	RuntimeFilter	   *plan;
	Plan			   *subplan;

	/* RuntimeFilter doesn't project, so tlist requirements pass through */
	subplan = create_plan_recurse(root, best_path->subpath, 0);

	plan = make_runtime_filter(subplan);
...
create_splitupdate_plan(PlannerInfo *root, SplitUpdatePath *path)
{
	...
	RelOptInfo *relOptInfo = root->simple_rel_array[path->resultRelation];
	Assert(relOptInfo);
...
RuntimeFilter *
make_runtime_filter(Plan *lefttree)
{
	RuntimeFilter  *node = makeNode(RuntimeFilter);
	Plan		   *plan = &node->plan;
