						 ...
						 Relation target_relation)
{
		...
		relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		/*
		 * GPDB_14_MERGE_FIXME: We need wholerow when updating AO table using pg14 optimizer,
		 * this can avoid extra seqscan in ExecUpdate node. However, the ORCA optimizer is not
		 * updated consistent with pg14, we still can use planSlot instead of using extra
		 * seqscan in ExecUpdate. As a result, if the targetlist contains all the columns of
		 * relation, we don't need wholerow junk attr.
		 *
		 * Maybe we need fix this problem complete in the future by redesigning AO/AOCS storage
