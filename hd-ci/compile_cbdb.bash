...
function upload_external_fts_cbdb_tar_and_rpm_package() {
	fts_mode=$1
	generate_docker_tag=$2
	tar_package_path=$(ls "${ROOT_PATH}"/cloudberrydb/gpdb_artifacts/server-build*)
	...
	cp $tar_package_path /tmp/$tar_upload_file_name
	...
	cp $rpm_package_path /tmp/$rpm_upload_file_name
	if [[ ${fts_mode} == "internal_fts" ]]; then
		cat <<EOF >>"${SRC_PATH}/cbdb-artifacts.txt"
os_type=${OS_TYPE_EXT}
os_arch=${OS_ARCH}
...
function release_rpm_external_fts() {
	...
	package_create_rpm 
	upload_external_fts_cbdb_tar_and_rpm_package "external_fts" ${gen_docker_tag}
}
