...
{
	DefineCustomStringVariable("unionstore.pageserver_connstring",
	...
	DefineCustomStringVariable("unionstore.safekeeper_token_env",
                               "the environment variable containing JWT token for authentication with Safekeepers, the convention is to either unset or set to $UNIONSTORE_AUTH_TOKEN",
                               NULL,
	...
	DefineCustomStringVariable("unionstore.timeline_id",
							   "unionstore timeline_id the server is running on",
							   NULL,
							   &neon_timeline,
							   "",
							   PGC_POSTMASTER,
	...
	DefineCustomStringVariable("unionstore.tenant_id",
							   "unionstore tenant_id the server is running on",
							   NULL,
							   &neon_tenant,
							   "",
							   PGC_POSTMASTER,
	...
	DefineCustomIntVariable("unionstore.max_cluster_size",
							"cluster size limit",
							NULL,
							&max_cluster_size,
							-1, -1, INT_MAX,
							...
							GUC_UNIT_MB,
							NULL, NULL, NULL);
	DefineCustomIntVariable("unionstore.flush_output_after",
							...
							0,	/* no flags required */
							NULL, NULL, NULL);
	DefineCustomIntVariable("unionstore.readahead_buffer_size",
	...
	DefineCustomIntVariable("unionstore.lsn_cache_size",
							"Size of last written LSN cache used by unionstore.",
                            NULL,
	...
	if (page_server_connstring && page_server_connstring[0])
	{
		...
		neon_prev_smgr_hook = smgr_hook;
		smgr_hook = neon_smgr_init;
		smgr_init_hook = smgr_init_neon;

		/* hook for index_create */
		neon_prev_index_create = index_create_hook;
        index_create_hook = neon_index_create;

        /* hook for index build */
        neon_prev_index_build = index_build_hook;
        index_build_hook = neon_index_build;

        /* hook for relation size calculation */
        neon_prev_relation_size = relation_size_hook;
        relation_size_hook = neon_relation_size;
	}
	lfc_init();
}
