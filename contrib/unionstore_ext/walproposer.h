...
typedef enum
{
	...
	/*
	 * Inactive state for the follower, we could not write any WAL to the follower safekeeper.
	 * When the follower safekeeper promoted to be leader, the state transformed to active.
	 */
	SS_INACTIVE,
}			SafekeeperState;
...
typedef struct AcceptorGreeting
{
	...
	term_t		term;
	NNodeId		nodeId;
	NNodeId     leader_nodeId;
	XLogRecPtr  flush_lsn;
	bool        am_safekeeper_leader;
}			AcceptorGreeting;
