-- In the qilin system, the output line number is inconsistent.
-- Here, the line number of the code is replaced with `###` by matching
--
-- start_matchsubs
-- m/\(pg_aovisimap_\d+\)/
-- s/\(pg_aovisimap_\d+\)/\(pg_aovisimap_###\)/
-- end_matchsubs
--
-- DEPENDENCIES
--
...
CREATE TABLE deptest (f1 serial, f2 text) DISTRIBUTED BY (f1);
...
CREATE TABLE deptest1 (f1 int) DISTRIBUTED BY (f1);
GRANT ALL ON deptest1 TO regress_dep_user1 WITH GRANT OPTION;

SET SESSION AUTHORIZATION regress_dep_user1;
CREATE TABLE deptest (a serial, b text) DISTRIBUTED BY (a);
...
CREATE TABLE deptest (a serial, b text);
