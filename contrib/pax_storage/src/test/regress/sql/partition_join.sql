...
-- GPDB_13_MERGE_FIXME: The query uses GPDB specific plan to do semi join.
-- The logic behind it is to convert the semi join to inner join and
-- deduplicate the rows. I attempt to implement right semi join for hash
-- join, like right out join to left out join. The problem is that the
-- inner table to build hash table on segment may overlap with other
-- segments. Shall we deserve to implement right semi join correctly?
...
-- GPDB_13_MERGE_FIXME: The total cost of MergeJoin is much less than one of
-- its children, while the hash join must consider all total costs of its
-- children. The instinct is that merge join with 2 SORT operators for the
-- tables should be slower than hash join. Not sure how it happens.
-- We should review the plans after MERGE.
