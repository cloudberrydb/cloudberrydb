...
gopherCreateConfig(DfsOption *option, const char *path)
{
    ...
    config->connect_path = (char *) GetGopherSocketPath();
...
makeConnection(ConnCacheEntry *entry, const char *serverName, const char *path)
{
    ...
    user = GetStorageUserMapping(GetUserId(), server->serverid);
    ...
    entry->hashvalue = GetSysCacheHashValue1(STORAGEUSERMAPPINGOID, ObjectIdGetDatum(user->umid));
...
invalCallback(Datum arg, int cacheId, uint32 hashvalue)
{
    ...
    Assert(cacheId == STORAGEUSERMAPPINGOID);
    ...
    while ((entry = (ConnCacheEntry *) hash_seq_search(&scan)))
    {
        ...
        if (hashvalue == 0 ||
            (cacheId == STORAGEUSERMAPPINGOID && entry->hashvalue == hashvalue))
            entry->invalidated = true;
    }
}

gopherFS
RemoteFileGetConnection(const char *serverName, const char *path)
{
    bool found;
    ConnCacheEntry *entry;
    ...
    if (connectionHash == NULL)
    {
        ...
        CacheRegisterSyscacheCallback(STORAGEUSERMAPPINGOID, invalCallback, (Datum) 0);
    }
