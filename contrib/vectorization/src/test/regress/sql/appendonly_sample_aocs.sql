...
CREATE TABLE test_tablesample_append (dist int, id int, name text) WITH (appendonly=true)  DISTRIBUTED BY (dist);
...
CREATE TABLE ttr1_append (a int, b int)  with(appendonly=true)   DISTRIBUTED BY (a);
CREATE TABLE ttr2_append (a int, b int)  with(appendonly=true)  DISTRIBUTED BY (a);
