...
static bool TargetListVecSupportsBackwardScan(List *targetlist);
static bool IndexVecSupportsBackwardScan(Oid indexid);
 ...
 */
void
ExecVecRestrPos(PlanState *node)
{
	switch (nodeTag(node))
	{
		case T_IndexScanState:
 ...
 */
bool
ExecVecSupportsMarkRestore(Path *pathnode)
{
	/*
 ...
 */
bool
ExecVecSupportsBackwardScan(Plan *node)
{
	if (node == NULL)
		return false;
	...
	switch (nodeTag(node))
	{
		case T_Result:
			if (outerPlan(node) != NULL)
				return ExecVecSupportsBackwardScan(outerPlan(node));
			else
				return false;
		...
		case T_Append:
			{
				...
				{
					if (!ExecVecSupportsBackwardScan((Plan *) lfirst(l)))
						return false;
				}
		...
		case T_IndexScan:
			return IndexVecSupportsBackwardScan(((IndexScan *) node)->indexid);

		case T_IndexOnlyScan:
			return IndexVecSupportsBackwardScan(((IndexOnlyScan *) node)->indexid);

		case T_SubqueryScan:
			return ExecVecSupportsBackwardScan(((SubqueryScan *) node)->subplan);
		...
		case T_WorkTableScan:
			return TargetListVecSupportsBackwardScan(node->targetlist);
		case T_Material:
		case T_Sort:
 ...
 */
void
ExecVecSquelchNode(PlanState *node)
{
	ListCell   *lc;
 ...
 */
static bool
TargetListVecSupportsBackwardScan(List *targetlist)
{
 ...
 */
static bool
IndexVecSupportsBackwardScan(Oid indexid)
{
	bool		result;
	HeapTuple	ht_idxrel;
 ...
 */
bool
ExecVecMaterializesOutput(NodeTag plantype)
{
	switch (plantype)
	{
		case T_Material:
