...
function gen_env() {
    cat > $tmpdir/run_test.sh <<-EOF
        ...
        ./test_binary_swap.sh -b /tmp/local/cloudberry-db-devel \
...
function install_gpdb_binary_swap() {
    [ ! -d /tmp/local/cloudberry-db-devel ] && mkdir -p /tmp/local/cloudberry-db-devel
    tar -xzf binary_swap_gpdb/bin_gpdb.tar.gz -C /tmp/local/cloudberry-db-devel
    sed -i -e "s/\/usr\/local/\/tmp\/local/" /tmp/local/cloudberry-db-devel/greenplum_path.sh
