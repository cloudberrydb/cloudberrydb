...
#		psql -d $DBNAME -c "select run_algebrized_query('$GPOPTUTILS_NAMESPACE', '$query_file', '$query')"
		psql -d $DBNAME -c "select run_optimize_test('$GPOPTUTILS_NAMESPACE', '$query_file', '$query')"
...
# The following tests exercise ONLY the optimizer 
psql -d $DBNAME -c "(select 'OK' as result, id as query, gpdb as \"native gpdb\", optimizerresult as \"optimize\" from dxl_tests where gpdb=optimizerresult) union all (select 'FAILED' as result, id as query, gpdb as \"native gpdb\", optimizerresult as \"optimize\" from dxl_tests where gpdb!=optimizerresult)  order by query;"
psql -d $DBNAME -c "(select 'OK' as result, count(*) as cnt from dxl_tests where gpdb=optimizerresult) union all (select 'FAILED' as result, count(*) as cnt from dxl_tests where gpdb!=optimizerresult);"
